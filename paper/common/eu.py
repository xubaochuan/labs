...
def load_all_data(filepath = dir_name + '/data.txt'):
    fr = open(filepath)
    label_list = []
    data_list = []
...
def load_stop_words(filepath = dir_name + '/smart_stop_words'):
    stop_words = set()
    fr = open(filepath)
    for line in fr.readlines():
        if line.strip() == '':
...
def shuffle(list1, list2, mode = 'load'):
    perm_path = dir_name + '/perm.txt'
    if mode == 'load' and os.path.exists(perm_path):
        fr = open(perm_path)
